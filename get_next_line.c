...
char	*prepare_previous_string(char *buf, char *line)
{
	int		i;
...
char	*finalize_line_prepare_buf(char *temp, char *buf, char *line)
{
	char	*final_line;
	int		i;
...
char	*realloc_and_copy_line(char *line, char *temp, size_t buf_size)
{
	char	*new_line;
	size_t	line_len;
	size_t	new_line_len;
...
int	g_n_l_return(int fd, char **line, char *buf)
{
